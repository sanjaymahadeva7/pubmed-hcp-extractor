...
class ExtractionLogger:
    def __init__(self, log_filename='extraction_log.txt'):
    ...
    def start_logging(self, config):
        ...
        self.log(f"PUBMED CONTACT EXTRACTION LOG")
        ...
        if config.get('date_range', {}).get('enabled', False):
            ...
            self.log(f"  Date Range: All dates")
    ...
    def log_contact_extracted(self, contact_data, affiliation_text):
        """Log successfully extracted contact"""
        self.log(f"      CONTACT EXTRACTED:")
        ...
        self.log(f"      Areas Of Expertise: {str(contact_data.get('Areas Of Expertise', 'N/A'))[:80]}...")
    ...
    def log_contact_skipped(self, author_name, reason):
        """Log skipped contact with reason"""
    ...
    def log_extraction_summary(self, stats):
        ...
        self.log(f"  Contacts Skipped (No Email): {stats.get('skipped_no_email', 0)}")
        self.log(f"  Contacts Skipped (Country Filter): {stats.get('skipped_country', 0)}")
        self.log(f"  Contacts Skipped (No Affiliation): {stats.get('skipped_no_affiliation', 0)}")
    ...
    def close(self):
        ...
        if self.log_file:
            self.log_file.close()
            print(f"\n Detailed log saved: {self.log_filename}")